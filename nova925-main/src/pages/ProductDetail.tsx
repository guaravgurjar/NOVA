...
export function ProductDetail() {
  ...
  usePageSEO({
    title: product ? `${product.name} — 925 Sterling Silver` : 'Product Not Found',
    description: product
      ? `Buy ${product.name} online at NOVA Jewellery. Premium 925 hallmarked sterling silver. Price: ₹${product.price.toLocaleString('en-IN')}. Free delivery & cash on delivery available.`
      : undefined,
    keywords: product ? `buy ${product.name.toLowerCase()}, 925 sterling silver ${product.name.toLowerCase()}, hallmarked silver ${product.category}` : undefined,
    canonicalPath: product ? `/product/${product.id}` : undefined,
    ogImage: product && product.images && product.images.length > 0 ? product.images[0] : undefined,
    twitterImage: product && product.images && product.images.length > 0 ? product.images[0] : undefined,
    ogType: 'product',
    noIndex: !product,
    jsonLd: product
      ? {
  ...
  const checkDelivery = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
      ...
      <div className="container mx-auto px-6 md:px-12 py-8 max-w-7xl flex-1">
        ...
        {relatedProducts.length > 0 && (
          <div className="border-t border-white/5 pt-16">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-2.5 md:gap-8">
