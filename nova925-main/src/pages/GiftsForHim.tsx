...
export function GiftsForHim() {
  usePageSEO({ title: 'Gifts For Him', description: 'Discover the perfect gift for him — 925 sterling silver rings, bracelets, chains & ear studs from NOVA.' });
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-nova-dark font-sans">
      ...
      {/* ─── Hero Banner ─────────────────────────────────────────────────── */}
      <div className="relative w-full aspect-[6250/1953] overflow-hidden border-b border-nova-gold/20 shadow-2xl flex items-center">
        <img
      ...
      <section className="container mx-auto px-6 md:px-12 py-12 max-w-7xl flex-1">
        <div className="flex items-center justify-between pb-6 mb-8 border-b border-white/10">
          <div className="flex items-center gap-3">
            <CircleDot className="w-4 h-4 text-nova-gold animate-pulse" />
          ...
          {selectedCategory !== 'all' && (
            <button
              ...
              className="text-xs text-nova-gold hover:text-nova-gold-light hover:underline font-medium transition-colors cursor-pointer"
            >
              Clear filter
            </button>
          )}
